@section('pageTitle', 'Offer Courses')
...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        {{ Breadcrumbs::render('offer_courses.index') }}
        <div class="">
            <div class="flex flex-row sm:justify-end mb-3 px-4 sm:px-0 -mr-2 sm:-mr-3">
                <div class="order-5 sm:order-6 mr-2 sm:mr-3">
                    <x-button-link href="{{ route('offer_courses.create') }}">
                        <i class="fa fa-plus"></i> {{ __('Offer New Course') }}
                    </x-button-link>
                </div>
            </div>
                ...
                <table class="border-collapse table-auto w-full bg-white table-striped relative">
                    ...
                    <tbody>
                        @foreach ($courses as $course)
                            ...
                            <tr data-school="{{ $course->program->department->school_id }}" data-department="{{ $course->program->department_id }}" data-program="{{ $course->program_id }}" data-semester="{{ $course->semester }}">
                                ...
                                <td class="border-b border-gray-100 px-6 py-3 text-gray-500 font-medium tracking-wide uppercase text-xs truncate">{{ $course->program->department->name }}</td>
                                <td class="border-b border-gray-100 px-6 py-3 text-gray-500 font-medium tracking-wide uppercase text-xs truncate">{{ $course->program->department->school->name }}</td>
                                <td class="border-b border-gray-100 px-6 py-3 text-gray-500 font-medium tracking-wide uppercase text-xs truncate">{{ $course->program->name }}</td>
                                ...
                                <td class="border-b border-gray-100 px-6 py-3 text-gray-500 font-medium tracking-wide uppercase text-xs truncate">
                                    <a href="{{ route('offer_courses.edit', $course) }}" class="text-blue-600 hover:text-blue-900 mr-2">{{ __('Edit') }}</a>
                                    <form action="{{ route('offer_courses.destroy', $course->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-900">{{ __('WITHDRAW') }}</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
