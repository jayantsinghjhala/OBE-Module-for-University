...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        {{ Breadcrumbs::render('course_outcomes.show', $course) }}
        <div class="flex flex-row mb-3 px-4 -mr-2 sm:-mr-3 sm:px-0  sm:justify-end">
            <div class="order-5 sm:order-6 mr-2 sm:mr-3">
                <x-button-link href="{{ route('course_outcomes.edit', $course) }}">
                    <i class="fa fa-edit"></i> {{ __('Edit') }}
                </x-button-link>
            </div>
            <div class="order-5 sm:order-6 mr-2 sm:mr-3">
                <form action="{{route('course_outcomes.destroy', $course->id)}}" method="post" class="py-2">
                    @csrf
                    @method('delete')
            ...
            <div class="p-6 bg-white border-b border-gray-200">
                @php
                    $courseInfo = [
                        ...
                        'Program' => $course->Program()->find($course->program_id)->name,
                        'Course Department' => $course->program->department->name,
                        'Course School' => ucfirst($course->program->department->school->name),
