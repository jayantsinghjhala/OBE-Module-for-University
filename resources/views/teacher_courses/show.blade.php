...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        {{ Breadcrumbs::render('teacher_courses.show', $course) }}
        <div class="flex flex-row mb-3 px-4 -mr-2 sm:-mr-3 sm:px-0  sm:justify-end">
            <div class="order-5 sm:order-6 mr-2 sm:mr-3">
                <x-button-link href="{{ route('teacher_courses.edit', $course) }}">
                    <i class="fa fa-edit"></i> {{ __('Edit') }}
                </x-button-link>
            </div>
            <div class="order-5 sm:order-6 mr-2 sm:mr-3">
                <form action="{{route('teacher_courses.destroy', $course->id)}}" method="post" class="py-2">
                    @csrf
                    @method('delete')
