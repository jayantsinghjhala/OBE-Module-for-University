@section('pageTitle', 'Create New Course Outcome')

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Create New Course Outcome') }}
        </h2>
    </x-slot>

    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        {{ Breadcrumbs::render('course_outcomes.create',$course) }}
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('course_outcomes.store',$course) }}">
                    @csrf
                    ...
                    <div class="form-control w-full p-3">
                        ...
                        <select name="name" class="select select-bordered w-full max-w-xs">
                            @for ($i = 1; $i <= 20; $i++)
                                <option value="CO{{ $i }}" {{ old('name') == 'CO'.$i ? 'selected' : '' }}>CO{{ $i }}</option>
                            @endfor
                        </select>
